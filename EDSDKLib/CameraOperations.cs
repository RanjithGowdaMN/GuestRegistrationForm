...
namespace CanonEDLibrary
{
    public class CameraOperations
    {
        ...
        public CameraOperations()
        {
            ...
            //LiveViewPicBox.Paint += LiveViewPicBox_Paint; TBD to main form
        ...
        private void MainCamera_StateChanged(Camera sender, StateEventID eventID, int parameter)
        {
            try { 
                ...
                {
                    //Invoke((Action)delegate { CloseSession(); }); 
                    CloseSession();
                } 
            }
        ...
        {
            try { 
                //Invoke((Action)delegate { MainProgressBar.Value = progress; }); 
            }
            catch (Exception ex) { ReportError(ex.Message, false); }
        }
        ...
        private void LiveViewPicBox_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (!MainCamera.IsLiveViewOn) e.Graphics.Clear(Color.Black);
            else
            {
                lock (LvLock)
                {
        ...
        */
        ...
        private void OpenSession()
        {
            ...
            MainCamera = CamList[0];//CamList[CameraListBox.SelectedIndex];
